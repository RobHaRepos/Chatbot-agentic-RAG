...
FastAPI-based vector retrieval service using FAISS for semantic search. Supports multiple vector stores, document management, prompt templates, and configurable chunking.

## Features

- **Multi-Store Support**: Create and manage multiple FAISS vector stores
- **Document Management**: Upload, update, and delete documents with automatic chunking
- **Prompt Templates**: Per-store customizable LLM prompt templates (auto-seeded on store creation)
- **Semantic Search**: Sentence-transformer embeddings for similarity search
- **SQLite Database**: Persistent storage for stores, documents, and templates
...
### Templates
- `GET /templates` - List templates (filter by `store_id`, `template_type`)
...
PATH_TO_FAISS_INDEX=./data/stores/default_store  # Default store path
...
```
data/stores/
├── default_store/    # Default store (ID 1)
│   ├── index.faiss
│   └── index.pkl
