...
from app.langgraph_code.src import wf_api
...
async def test_lifespan_context_manager(monkeypatch):
    ...
    monkeypatch.setattr("app.langgraph_code.src.wf_api.build_workflow", lambda: FakeGraph())
...
def test_get_ready_happy(monkeypatch):
    ...
    monkeypatch.setattr("app.langgraph_code.src.wf_api.app.state.graph", object(), raising=False)
...
def test_get_ready_sad(monkeypatch):
    ...
    monkeypatch.setattr("app.langgraph_code.src.wf_api.app.state.graph", None, raising=False)
    response = wf_api.ready_check()
    assert isinstance(response, dict)
    assert "status" in response
    assert isinstance(response["status"], bool)
...
async def test_run_workflow_happy(monkeypatch):
    ...
    monkeypatch.setattr("app.langgraph_code.src.wf_api.app.state.graph", fake_graph, raising=False)
...
async def test_run_workflow_invoke_exception(monkeypatch):
    ...
    monkeypatch.setattr("app.langgraph_code.src.wf_api.app.state.graph", BrokenGraph(), raising=False)
